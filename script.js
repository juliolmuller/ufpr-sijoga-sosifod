...
var tipo = window.location.search.substr(1).split('&')[0].split('=')[1];
var usuario = window.location.search.substr(1).split('&')[1].split('=')[1];
var senha = window.location.search.substr(1).split('&')[2].split('=')[1];

function init() {
    tipo = window.location.search.substr(1).split('&')[0].split('=')[1];
    usuario = window.location.search.substr(1).split('&')[1].split('=')[1];
    senha = window.location.search.substr(1).split('&')[2].split('=')[1];
    console.log(tipo);
    console.log(usuario);
    console.log(senha);
    ...
    var i=0;
      ...
      if(item.numero != "" && item.situacao != "" && item.promovido != "" && item.promovente != ""){
          if(((item.advpromovido == usuario || item.advpromovente == usuario) && tipo == "Advogado") || (((item.promovido == usuario || item.promovente == usuario) && item.situacao != "Encerrado") && tipo == "Parte") || (item.juiz == usuario && tipo =="Juiz")){ 
            ...
            parsed +="<span class='juiz'><b>Juiz:</b> "+item.juiz+"</span>"; 
            parsed +="</div>";
            parsed +="<div class='partes'>";
            parsed +="<span class='promovido'><b>Promovido:</b> "+item.promovido+"<span style='margin-left: 25px;'><b>Advogado:</b> "+item.advpromovido+"</span>"; 
            parsed +="</div>";
            parsed +="<div class='partes'>";
            parsed +="<span class='promovente'><b>Promovente:</b> "+item.promovente+"<span style='margin-left: 25px;'><b>Advogado:</b> "+item.advpromovente+"</span>"; 
            parsed +="</div>";
            parsed +="<div class='descricao'>"+item.descricao+"</div>";
            parsed +="</div>";         
          }
        }
  	});                       
